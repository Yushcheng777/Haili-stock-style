...
AKSHARE_IMPORT_PATTERN = re.compile(r'^\s*import\s+akshare\s+as\s+ak', re.MULTILINE)
...
def replace_in_file(file_path, dry_run=False):
    with open(file_path, encoding="utf-8") as f:
        orig = f.read()
    if "akshare" not in orig:
        return False
    new_content = transform_content(orig)
    if not dry_run:
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(new_content)
    print(f"Replaced akshare with tushare in: {file_path}")
    return True
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Replace akshare with tushare in all .py files")
